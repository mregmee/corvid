...
void Trainer::train()
{
	...
	*/
	...
	for(classIterator=classes.begin();classIterator!=classes.end();++classIterator)
	{
		...
		cout<<"rule mining complete"<<endl;
		for(int i=0;i<classSize;++i)
		{
		cout<<"class: "<<i<<endl;
			int k=0;
			map< vector<int>,statistic* >::iterator ruleItr;
			for(ruleItr=frequentRules[i].begin();ruleItr!=frequentRules[i].end();++ruleItr)
			{
				cout<<"rule  "<<k<<"  ";
				for(int j=0;j<ruleItr->first.size();++j)
					cout<<ruleItr->first[j]<<"\t";
				cout<<"count: "<<ruleItr->second->count<<"\t"<<"support: "<<ruleItr->second->sup<<endl;
				k++;
			}
		...
		{
			cout<<"class: "<<i<<endl<<endl;
			for(int j=0;j<rules[i].size();++j)
			{
				cout<<"rule: "<<j<<"\t";
				for(int k=0;k<rules[i][j]->pattern.size();++k)
					cout<<rules[i][j]->pattern[k]<<" ";
				cout<<endl;
			}
		}
