...
private:
	...
	float distribution;
	int resultClass;
	int sz;//size of transaction set of email being classified
	vector<float> resultSet;//probability distribution of classification
public:
	...
	int getResultClass(){ return resultClass;}
};
...
void Classifier::processEmail()
{
	...
	transactions.createTransactions(preProcessor->getSentences(),preProcessor->getSubject(),transactionSet,false);
	//Transactions::reset();	
	...
	cout<<"constructing patterns---------------------------------------------------------------"<<endl;
...
string  Classifier::classify( )
{
	...
	resultSet.push_back(result[0]/total);
	for(int i=1;i<classSize;++i)
	{
		if(result[i]>result[resultClass])
			resultClass=i;	
		resultSet.push_back(result[i]/total);
	}
