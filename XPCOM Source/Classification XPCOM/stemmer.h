// stemtest.cpp : Defines the entry point for the console application.
//
...
void stemfile(vector< vector<string> > & sentences)
{
	...
	for(int i=0;i<sentences.size();i++)
	{
		...
		{
			string str=sentences[i][j];			
			const char* st=str.c_str();
			char* cp=new char[str.length()+1];
			strcpy(cp,st);	
			int p=stem(z, cp, str.length()-1);
			p++;
			sentences[i][j]=str.substr(0,p);		
		}
	}
